...
\begin{document}
\title{\textbf{biblatex 参考文献和引用样式：\texttt{caspervector} v0.1.7}}
\author{%
...
版权所有 \copyright\ 2012-2013 Casper Ti.\ Vector。%
...
使用 biblatex 2.6 和 biber 1.6（或两者的更高版本）的用户可以通过
...
biber 1.6 之前的版本存在一个 bug\footnote{\ %
	\url{https://github.com/plk/biblatex/issues/101}.%
}：在源代码中任意位置出现的 \verb|\nocite{*}| 都会覆盖整个引用顺序记录，
导致即使在 \verb|sorting = none| 的设定下，
排版出来的文献列表依然不是按照引用顺序排序的情况。
因此，如果需要使用相关功能，\myemph{强烈建议将 biblatex 和 biber 升级到最新版本}。
