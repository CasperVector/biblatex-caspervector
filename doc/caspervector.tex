...
用户如果需要更严格符合国家标准的样式，可以参考 \parencite{blx-gbt7714-2015}。
...
例如对于一个 \verb|@article| 类型的条目，
\begin{Verbatim}[frame = single]
year = {2018},
volume = {14},
number = {\hologo{TeX} 特刊},
\end{Verbatim}
将使年卷期号排版为类似于“\textbf{2018}，\textit{14}(\hologo{TeX} 特刊)”的形式。
...
其具体用法请参考 biblatex 手册\supercite{biblatex}）\footnote{\ %
	v0.2.x 和更早版本的 \texttt{caspervector} 样式支持直接在
	\texttt{year} 字段中设定特殊内容；在之后的版本中，为了支持著者--年编码制，
